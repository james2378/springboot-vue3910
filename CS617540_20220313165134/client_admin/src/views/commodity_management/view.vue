			...
			<el-col v-if="user_group === '管理员' || $check_field('get','commodity_number') || $check_field('add','commodity_number') || $check_field('set','commodity_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="商品编号" prop="commodity_number">
					<el-input id="commodity_number" v-model="form['commodity_number']" placeholder="请输入商品编号"
							  v-if="user_group === '管理员' || (form['commodity_management_id'] && $check_field('set','commodity_number')) || (!form['commodity_management_id'] && $check_field('add','commodity_number'))" :disabled="disabledObj['commodity_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','commodity_type') || $check_field('add','commodity_type') || $check_field('set','commodity_type')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="商品类型" prop="commodity_type">
					<el-input id="commodity_type" v-model="form['commodity_type']" placeholder="请输入商品类型"
							  v-if="user_group === '管理员' || (form['commodity_management_id'] && $check_field('set','commodity_type')) || (!form['commodity_management_id'] && $check_field('add','commodity_type'))" :disabled="disabledObj['commodity_type_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','inventory_quantity') || $check_field('add','inventory_quantity') || $check_field('set','inventory_quantity')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="库存数量" prop="inventory_quantity">
					<el-input-number id="inventory_quantity" v-model.number="form['inventory_quantity']"
						v-if="user_group === '管理员' || (form['commodity_management_id'] && $check_field('set','inventory_quantity')) || (!form['commodity_management_id'] && $check_field('add','inventory_quantity'))"></el-input-number>
