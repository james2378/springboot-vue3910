			...
			<el-col v-if="user_group === '管理员' || $check_field('get','commodity_number') || $check_field('add','commodity_number') || $check_field('set','commodity_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="商品编号" prop="commodity_number">
					<el-input id="commodity_number" v-model="form['commodity_number']" placeholder="请输入商品编号"
							  v-if="user_group === '管理员' || (form['purchase_registration_id'] && $check_field('set','commodity_number')) || (!form['purchase_registration_id'] && $check_field('add','commodity_number'))" :disabled="disabledObj['commodity_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','commodity_type') || $check_field('add','commodity_type') || $check_field('set','commodity_type')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="商品类型" prop="commodity_type">
					<el-input id="commodity_type" v-model="form['commodity_type']" placeholder="请输入商品类型"
							  v-if="user_group === '管理员' || (form['purchase_registration_id'] && $check_field('set','commodity_type')) || (!form['purchase_registration_id'] && $check_field('add','commodity_type'))" :disabled="disabledObj['commodity_type_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','trade_name') || $check_field('add','trade_name') || $check_field('set','trade_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="商品名称" prop="trade_name">
					<el-input id="trade_name" v-model="form['trade_name']" placeholder="请输入商品名称"
							  v-if="user_group === '管理员' || (form['purchase_registration_id'] && $check_field('set','trade_name')) || (!form['purchase_registration_id'] && $check_field('add','trade_name'))" :disabled="disabledObj['trade_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','purchase_quantity') || $check_field('add','purchase_quantity') || $check_field('set','purchase_quantity')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="进货数量" prop="purchase_quantity">
					<el-input-number id="purchase_quantity" v-model.number="form['purchase_quantity']"
						v-if="user_group === '管理员' || (form['purchase_registration_id'] && $check_field('set','purchase_quantity')) || (!form['purchase_registration_id'] && $check_field('add','purchase_quantity'))"></el-input-number>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','remarks') || $check_field('add','remarks') || $check_field('set','remarks')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="备注" prop="remarks">
					<el-input id="remarks" v-model="form['remarks']" placeholder="请输入备注"
							  v-if="user_group === '管理员' || (form['purchase_registration_id'] && $check_field('set','remarks')) || (!form['purchase_registration_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "purchase_registration_id",
				url_add: "~/api/purchase_registration/add?",
				url_set: "~/api/purchase_registration/set?",
				url_get_obj: "~/api/purchase_registration/get_obj?",
				url_upload: "~/api/purchase_registration/upload?",

				query: {
					"purchase_registration_id": 0,
				},
				...
				form: {
					...
					"commodity_type":'', // 商品类型
					"trade_name":'', // 商品名称
					"purchase_quantity":0, // 进货数量
					"remarks":'', // 备注
					"purchase_registration_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/purchase_registration/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/purchase_registration/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/purchase_registration/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/purchase_registration/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/purchase_registration/view','get');
