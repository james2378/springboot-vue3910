			...
			<el-col v-if="user_group === '管理员' || $check_field('get','remarks') || $check_field('add','remarks') || $check_field('set','remarks')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="备注" prop="remarks">
					<el-input id="remarks" v-model="form['remarks']" placeholder="请输入备注"
							  v-if="user_group === '管理员' || (form['sales_system_id'] && $check_field('set','remarks')) || (!form['sales_system_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/sales_system/add?",
				url_set: "~/api/sales_system/set?",
				url_get_obj: "~/api/sales_system/get_obj?",
				url_upload: "~/api/sales_system/upload?",
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/sales_system/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/sales_system/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/sales_system/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/sales_system/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/sales_system/view','get');
            ...
            submit_after(){
                ...
                if (this.form[this.field]){
                    ...
                    this.$get("~/api/sales_system/get_obj?",query_form,(res)=>{
            ...
            submit_goods(source_id,add_flag){
                ...
                if (add_flag==='set'){
                    ...
                    // 提交事件
                    this.$post("~/api/goods/set?source_table=sales_system&source_field=" + this.field + "&source_id=" + source_id,this.goods_form,(res)=>{
