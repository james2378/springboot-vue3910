				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/member_user/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_id: this.obj["member_user_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/member_user/set?member_user_id=' + _this.obj["member_user_id"], {
								praise_len
							}, (res) => {
								if(res.result){
