<template>
	<div class="diy_details_box diy_div_sales_registration">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "sales_registration_id",
					source_id: this.obj["sales_registration_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/sales_registration/set?sales_registration_id=' + _this.obj["sales_registration_id"], {
								praise_len
							}, (res) => {
								if(res.result){
